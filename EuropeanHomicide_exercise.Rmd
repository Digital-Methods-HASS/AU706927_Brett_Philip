...
date: " created 4 October 2021, updated 24 november 2022"
output:
html_document:
    toc: true
    toc_float: true
...
ggplot(data = Western_Europe) + 
  ...
  labs(title = "Homicides Western Europe") + 
  labs(x = "Year", y = "Number of Homicides per 100,000 people", title = "Homicide rate in Europe from 1300-2000")
```
...
-   How would you characterize the trend compared to the homicides above?
    ...
    We can see that the monarchs are ruling for a longer and longer duration. The homicide trend is reversed, so that the rate of homicides is falling. So the two trends combine nicely.
...
kings_2 %>% 
  ...
  geom_smooth() + 
  labs(title = "Reign duration", x = "Midyear", y = "Duration")
```
